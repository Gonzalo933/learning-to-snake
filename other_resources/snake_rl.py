...
downsampling = 2
D = int((150 / downsampling) * (150 / downsampling))  # input dimensionality: 80x80 grid
if resume:
    model = pickle.load(open("save.p", "rb"))
else:
    model = {}
    model["W1"] = np.random.randn(H, D) / np.sqrt(D)  # "Xavier" initialization
    model["W2"] = np.random.randn(n_classes, H) / np.sqrt(H)
...
def preprocess(I):
    I = I[::downsampling, ::downsampling, :]  # downsample by factor of 2
    I[I[:, :, 2] == 255] = 1  # Food
    I[I[:, :, 0] == 255] = 1  # Snake head
    I[I[:, :, 0] == 1] = 1  # Snake torso
    I[I[:, :, 1] == 255] = 0  # Background
    I = I[:, :, 0]
...
while True:
    ...
    y = np.zeros(n_classes)  # 1 if action == 2 else 0  # a "fake label"
    y[action] = 1
